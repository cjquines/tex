...
\begin{thmboxed*}
\thmrating{Anti-Schur's theorem}{1}{1}
...
Given a sequence $\seq{a_n}$, we define its \textbf{Zsigmondy set} $\zsigseq{a_n}$ as the set of $n \ge 1$ such that $a_n$ \textit{doesn't} have a primitive prime divisor. In other words, $n \in \zsigseq{a_n}$ if $p \mid a_n$ implies $p \mid a_k$ for some $k < n$. Then:
...
\begin{thmboxed*}
\thmrating{Sylvester--Schur theorem}{4}{1}
...
\begin{thmboxed*}
\thmrating{Dirichlet's theorem (density)}{3}{3}
