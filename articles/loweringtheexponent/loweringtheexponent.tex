...
\item We show sequence is constant. Scale to remove common factors, then take $p \mid a_1$. Take $(m, n) = (1, d + 1)$ and $(2, d + 2)$ and divide the geometric mean conditions to get $ d \mid \nu_p( a_{d + 1} ) - \nu_p( a_1 ) $. For arbitrarily large $d$, RHS must be zero, so $\nu_p(a_{d+1}) = \nu_p(a_1) > 0$, so the sequence is eventually $0$ mod $p$. But from $(m, n) = (i, p + i)$, the sum of every $p$ consecutive terms is $0$ mod $p$, so the whole sequence is $0$ mod $p$, contradiction.

\item If $\nu_p(a), \nu_p(b), \nu_p(c)$ are all equal, then by $abc = 1$ they're all zero. Take $\nu_p(a) > 0$, WLOG $\nu_p(b) < 0$. If $\nu_p(c) \ge 0$, then $\nu_p(a^x + b^y + c^z) = \nu_p(b^y) < 0$, so it can't be an integer; thus $\nu_p(c) < 0$. Similarly, if $y \nu_p(b) \ne z \nu_p(c)$, we get $\nu_p(a^x + b^y + c^z) = \min\cbr{ y\nu_p(b), z \nu_p(c) } < 0$. Because $y \nu_p(b) = z \nu_p(c)$, we have some $d, y', z'$ such that $\nu_p(b) = -dz'$, $\nu_p(c) = -dy'$, and $\nu_p(a) = d(y' + z')$, which makes it powerful.

\item Let $p := 3$. Suppose they were equal for $x, y$, we'll get a contradiction. WLOG, $\nu_p(x) > 0 \ge \nu_p(y)$. Note $\nu_p(f(x)) = \nu_p(x^p + x) = \nu_p(x) > 0$, so $p \mid f^m(x)$. Check if $\nu_p(y) \le 0$ we get $p \nmid f^n(y)$, contradiction.

\item Only $g(n) = n + C$ works. We'll show $g$ is injective mod $p$. Suppose $p \mid g(b) - g(a)$. We choose $m$ to force $\nu_p(g(a) + m)$ and $\nu_p(g(b) + m)$ to be odd: if $p^2 \mid g(b) - g(a)$, take $m \equiv p - g(a) \pmod{p^2}$, otherwise take $m \equiv -g(b) \pmod{p^3}$. This shows $p \mid g(m) + a$ and $p \mid g(m) + b$, so $p \mid a - b$, done. From injectivity, we get $g(n + 1) - g(n) = \pm 1$. Taking $(m, n) = (n, n + 1)$ as a perfect square rules out the $-1$ case.
