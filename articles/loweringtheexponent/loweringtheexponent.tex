...
\begin{itemize}
  ...
  % TODO: equality is just false when p = 2?
...
\item Even integers work: $(1, n, 1)$ gives $2n$. We show odd is impossible. Let $a, b, c$ be $\nu_2(x), \nu_2(y), \nu_2(z)$, respectively. If expression was odd, we have $\min\cbr{ \max\cbr{a, b} + \max\cbr{b, c} } = \max\cbr{a, c}$. By symmetry, WLOG $a \le c$. Do casework on relative sizes of $\cbr{a, b, c}$. For the case where $b$ isn't between $a$ and $c$, we have to go back to the original expression to handle equality, which is weird for $\nu_2$.
