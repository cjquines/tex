...
\item Let $n \ge 3$. Positive integers $a_1, \ldots, a_n$ satisfy $(a_1, \ldots, a_n)[a_1, \ldots, a_n] = a_1 \cdots a_n$. Prove that $(a_i, a_j) = 1$ for all $i \ne j$.
% v_p, done
...
% i guess the motivation is "injective over p" => "consider v_p"
...
% A sequence of integers $\{a_k \}$ is good if and only if $m \mid a_m^2$ and $a_m \mid m^2$, for each $m \in \mathbb{N}$.
% (equiv to v_p(n)/2 <= v_p(a_n) <= 2v_p(n))
...
% the idea is really "function is determined by values at every prime exponent"
...
% in OTIS excerpts 2019, problem 157
...
% the spam also gives OTIS vibes sigh
...
% look at v_3. i suppose it's not *that* motivated to look at v_3 specifically
...
% seems like an OTIS problem based on posts lmao
% also, really more about bounding than it is about v_p
...
% another an algebraic symmetry manip kinda problem
